...
var jsonEnvio ={
  ...
  {method: 'POST', body: JSON.stringify(jsonEnvio),headers:{'Content-Type':'application/json','auth-token':generarToken() }}) 
    ...
    .then(datos => {
      ...
      //console.log(datos.data.payment.payment_url);
      console.log(req.body);
      //res.json(datos); 
      var sql = `INSERT INTO users 
      (
          id, created, nombre, apellido,link
      )
      VALUES
      (
          ?, ?, ?, ?, ?
      )`;
     conexion.query(sql, [datos.data.order.id , dateString, req.body.nombre, req.body.apellido,datos.data.payment.payment_url], function (err, data)  {
        if (err)
            throw err;
    });
